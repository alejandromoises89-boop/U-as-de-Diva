    ...
    <div className="w-full max-w-7xl mx-auto p-4 mb-20">
      ...
      {filteredServices.length === 0 ? (
        <div className="text-center py-20 bg-white rounded-3xl border-2 border-dashed border-brand-200">
           <p className="text-brand-400 font-sans italic text-lg">No encontramos el servicio solicitado. Prueba con otra palabra clave.</p>
        </div>
      ) : (
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-8 lg:gap-10">
          {filteredServices.map((item, index) => (
            <div 
              key={item.id}
              className="group relative flex flex-col h-full bg-white rounded-3xl overflow-hidden border border-brand-200 shadow-lg hover:shadow-2xl hover:border-gold-500/40 transition-all duration-500 hover:-translate-y-3 cursor-pointer active:scale-95 active:bg-brand-50 premium-card"
              onClick={() => onSelectService(item.id)}
              style={{ animationDelay: `${index * 100}ms` }}
            >
              <div className="relative h-80 overflow-hidden bg-brand-100 shimmer">
                <img 
                  src={item.image} 
                  alt={item.title}
                  title={item.image} // Requisite: Tooltip displaying the full image URL on hover
                  loading="lazy"
                ...
                <div className="absolute bottom-5 right-5 z-20">
                   <div className="bg-white/95 backdrop-blur-md text-brand-900 px-5 py-2.5 rounded-xl font-mono font-bold text-xl shadow-xl border border-brand-100 group-hover:bg-gold-500 group-hover:text-white transition-all duration-500">
                      {formatCurrency(item.price)}
                   </div>
                </div>
              </div>

              <div className="relative z-20 flex-1 flex flex-col p-8 bg-white transition-colors duration-300">
                ...
                <p className="text-brand-800 text-sm font-sans leading-relaxed mb-8 line-clamp-3 opacity-90 flex-1">
                  {item.description}
                </p>
                <div className="mt-auto pt-5 border-t border-brand-50 flex items-center justify-between text-[10px] font-black uppercase tracking-widest text-brand-500 group-hover:text-gold-600">
